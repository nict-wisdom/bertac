...
""" Modification for BERTAC
 The following functions have benn added/modified for BERTAC
...
class PreTrainedTokenizer(object):
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        for file_id, file_path in vocab_files.items():
            if file_path == resolved_vocab_files[file_id]:
                logger.info("loading file {} with id {}".format(file_path,file_id))
            else:
                logger.info("loading file {} from cache at {} with id {}".format(file_path, resolved_vocab_files[file_id], file_id))
    ...
    def tokenize(self, text, **kwargs):
        ...
        text = self.prepare_for_tokenization(text, **kwargs) # nothing but retun text in this implemenation
    ...
    # without wordpiece tokenize, only the basic tokenizer (punct accecnt splitting) is applied
    # addedd by Jong-Hoon Oh
    def tokenize_for_cnn(self, text, **kwargs):
        """ Converts a string in a sequence of tokens (string), using the tokenizer.
            Split in words for word-based vocabulary or sub-words for sub-word-based
            vocabularies (BPE/SentencePieces/WordPieces).
        ...
        def split_on_token_for_cnn(tok, text):
            result = []
            split_text = text.split(tok)
            for i, sub_text in enumerate(split_text):
                sub_text = sub_text.rstrip()
        ...
        def split_on_tokens_for_cnn(tok_list, text):
            if not text.strip():
                return []
            if not tok_list:
                return self._tokenize(text)
            ...
            for tok in tok_list:
                ...
                for sub_text in text_list:
                    if sub_text not in self.unique_added_tokens_encoder:
                        tokenized_text += split_on_token_for_cnn(tok, sub_text)
                    else:
                        tokenized_text += [sub_text]
                text_list = tokenized_text
            ...
            return list(
                itertools.chain.from_iterable(
                    (
                        self._tokenize_for_cnn(token) if token not in self.unique_added_tokens_encoder else [token]
                        for token in tokenized_text
                    )
                )
            )
        ...
        added_tokens = self.unique_added_tokens_encoder
        tokenized_text = split_on_tokens_for_cnn(added_tokens, text)
        return tokenized_text
    ...
    def _tokenize_for_cnn(self, text, **kwargs):
        """ Converts a string in a sequence of tokens (string), using the tokenizer.
            Split in words for word-based vocabulary or sub-words for sub-word-based
            vocabularies (BPE/SentencePieces/WordPieces).
    ...
    def convert_bert_tokens_to_original_tokens(self, tokens):
        ostr = []
        for token in tokens:
           if token != '##UU':
              ostr.append(token)
    ...
    # added by Jong-Hoon Oh
    def convert_original_tokens_to_cnn_ids(self, tokens, cnn_stoi):
        """ Converts a single token, or a sequence of tokens, (str) in a single integer id
            (resp. a sequence of ids), using the vocabulary cnn_stoi.
            here, tokens are assumed to be bert-tokenized words like 'trip ##ad ##vis ##or' for 'tripadvisor'
              where, the given 'tokens' contains 'trip ##ad ##vis ##or'
            So this four tokens 'trip ##ad ##vis ##or' are first converted into a word 'tripadvisor' then cnn_stoi is applied.
        """
        if tokens is None:
            return None
        if isinstance(tokens, str):
            if tokens in cnn_stoi:
               return cnn_stoi[tokens], tokens+"/"+str(cnn_stoi[tokens])
            return cnn_stoi['<unk>'], tokens+"/<unk>"

        ids = []
        atok = []
        ostr = self.convert_bert_tokens_to_original_tokens(tokens)

        for i in range(len(ostr)):
            tid = cnn_stoi[ostr[i]] if ostr[i] in cnn_stoi else cnn_stoi['<unk>']
            ids.append(tid)
            if ostr[i] in cnn_stoi:
               atok.append(ostr[i]+"/"+str(cnn_stoi[ostr[i]]))
            else:
               atok.append(ostr[i]+"/<unk>")
    ...
    def encode_for_cnn(
        ...
        **kwargs
        ...
        none_str = None
        # 'text' is a question string (or text_a in GLUE)
        qtokens_upper = self.tokenize_for_cnn(text, add_special_tokens=add_special_tokens, **kwargs) 
        encoded_inputs = self.encode_plus_for_cnn(
            text,
            none_str,
            qtokens_upper,
            cnn_stoi,
            text_pair1=text_pair,
            stokens=none_str,
            stokens_upper=none_str,
            max_length=max_length,
            add_special_tokens=add_special_tokens,
            stride=stride,
            truncation_strategy=truncation_strategy,
        ...
        return encoded_inputs["input_ids"], encoded_inputs["tokens"], encoded_inputs["tokens_upper"]

    # added by Jong-Hoon Oh
    def encode_plus_for_cnn(
        self,
        text1,
        ftokens,
        ftokens_upper,
        ...
        **kwargs
        ...
        #  - ftokens: question tokens or text_a tokens (by AlbertTokenizer)
        #  - cnn_stoi: stoi mapping table for CNN vocabularies
        #  - stokens: span (paragraph) tokens or text_b tokens (by AlbertTokenizer)

        def get_input_ids(text1):
            if isinstance(text1, str):
                # 'text1' is a qstr (should be tokenized)
                tokens = self.tokenize(text1, add_special_tokens=add_special_tokens, **kwargs) # raw text
                return tokens, self.convert_tokens_to_ids(tokens)
            elif isinstance(text1, (list, tuple)) and len(text1) > 0 and isinstance(text1[0], str): # already tokenized
                # 'text_pair1' is a paragraph string that is already tokenized
                return text1, self.convert_tokens_to_ids(text1)
            elif isinstance(text1, (list, tuple)) and len(text1) > 0 and isinstance(text1[0], int): # already id converted 
                return text1, text1
            else:
                raise ValueError(
        ...
        def get_cnn_ids(text1, cnn_stoi):
            if isinstance(text1, str):
                return self.convert_original_tokens_to_cnn_ids(text1, cnn_stoi)
            elif isinstance(text1, (list, tuple)) and len(text1) > 0 and isinstance(text1[0], str): # already tokenized
                return self.convert_original_tokens_to_cnn_ids(text1, cnn_stoi)
            else:
                raise ValueError(
        ...
        first_aids = None
        first_aids_debug = None
        first_tokens, first_ids = get_input_ids(text1)
        if text_pair1 is not None:
           # if it is not for question processing, truncated tokenized ftokens are used as first_tokens
           if ftokens is not None:
              first_tokens = ftokens 
           second_tokens, second_ids = get_input_ids(text_pair1)
           first_input = ftokens_upper
           second_input = stokens_upper
           # get_cnn_ids for the first and second tokens
           first_aids, first_aids_debug = get_cnn_ids(first_input, cnn_stoi)
           second_aids, second_aids_debug = get_cnn_ids(second_input, cnn_stoi)
        else:
           # processing for single sentence task
           first_input = ftokens_upper
           first_aids, first_aids_debug = get_cnn_ids(first_input, cnn_stoi)
           second_tokens = None
           second_ids = None
           second_aids = None
           second_aids_debug = None

        return self.prepare_for_model_for_cnn(
            first_ids,
            first_aids,
            first_aids_debug,
            first_tokens,
            ftokens_upper,
            pair_ids=second_ids,
            pair_aids=second_aids,
            pair_aids_debug=second_aids_debug,
            pair_tokens=second_tokens,
            pair_tokens_upper=stokens_upper,
    ...
    def prepare_for_model_for_cnn(
        ...
        pair_ids=None,
        pair_aids=None,
        pair_aids_debug=None,
        pair_tokens=None,
        pair_tokens_upper=None,
        pad_token_aid=None,
        max_length=None,
        add_special_tokens=True,
        stride=0,
        ...
        Args:
            ...
            aids: list of tokenized input ids. Can be obtained from a string by chaining the
                `tokenize` and `convert_tokens_to_cnn_ids` methods.
            aids_debug: aids for debugging
            ftokens: list of tokenized string of the first sentence (or question)
            ftokens_upper: The upper or lower case of ftokens
            pair_ids: Optional second list of input ids. Can be obtained from a string by chaining the
                `tokenize` and `convert_tokens_to_ids` methods.
            pair_aids: Optional second list of input ids. Can be obtained from a string by chaining the
                `tokenize` and `convert_tokens_to_cnn_ids` methods.
            pair_aids_debug: pair_aids for debugging
            pair_tokens: list of tokenized string of the sentence pair
            paier_tokens_upper: The upper or lower case of pair_tokens
        ...
        if max_length and total_len > max_length:
            # truncating
            # overflowing_tokens are reused after truncating
            ids, pair_ids, overflowing_tokens, overflowing_tokens_upper = self.truncate_sequences_for_cnn(
                ids,
                pair_ids=pair_ids,
                pair_tokens=pair_tokens,
                pair_tokens_upper=pair_tokens_upper,
                num_tokens_to_remove=total_len - max_length,
                truncation_strategy=truncation_strategy,
                stride=stride,
            )
            # adjust length of ids only if aids (pair_aids) are longer than truncated ids and pair_ids
            #  otherwise use as they are since they are padded with '<pad>''s id as shown below
            # ftokens are for truncated question strings, 
            #  so they should be truncated either if ids were truncated
            if len(ids) < len(ftokens):
               ftokens = ftokens[:len(ids)]
            if aids is not None:
               if len(ids) < len(aids):
                  aids = aids[:len(ids)]
            if pair_aids is not None:
               if len(pair_ids) < len(pair_aids):
                  pair_aids = pair_aids[:len(pair_ids)]

            if return_overflowing_tokens:
                encoded_inputs["overflowing_tokens"] = overflowing_tokens
                encoded_inputs["overflowing_tokens_upper"] = overflowing_tokens_upper
                encoded_inputs["num_truncated_tokens"] = total_len - max_length

        # modified by Jong-Hoon Oh
        # To deal with cnn_ids indenpdently from BERT/ALBERT ids (input_aids, amask) 
        # encoded_inputs["input_qids","input_pids","qmasks","pmasks"] are stored in encoded_inputs
        encoded_inputs["input_qids"] = aids 
        encoded_inputs["input_pids"] = pair_aids 
        encoded_inputs["input_qids_debug"] = aids_debug
        encoded_inputs["input_pids_debug"] = pair_aids_debug
        encoded_inputs["qmasks"] = None
        encoded_inputs["pmasks"] = None
        # truncating and padding
        if encoded_inputs["input_qids"] is not None:
           if(len(encoded_inputs["input_qids"]) > max_length): # if exceeds max_length
              encoded_inputs["input_qids"] = encoded_inputs["input_qids"][:max_length]
              encoded_inputs["qmasks"] = [1] * len(encoded_inputs["input_qids"])
           else: # otherwise: padding with 'pad_token_aid' for '<pad>'
              pad_token_aid = pad_token_aid if pad_token_aid is not None else 0
              difference = (max_length if max_length is not None else self.max_len) - len(encoded_inputs["input_qids"])
              encoded_inputs["qmasks"] = [1] * len(encoded_inputs["input_qids"]) + [0] * difference
              encoded_inputs["input_qids"] = encoded_inputs["input_qids"] + [pad_token_aid] * difference

        if encoded_inputs["input_pids"] is not None:
           if(len(encoded_inputs["input_pids"]) > max_length): #if exceeds max_length
              encoded_inputs["input_pids"] = encoded_inputs["input_pids"][:max_length]
              encoded_inputs["pmasks"] = [1] * len(encoded_inputs["input_pids"])
           else: # otherwise: padding with 'pad_token_aid' for '<pad>'
              pad_token_aid = pad_token_aid if pad_token_aid is not None else 0
              difference = (max_length if max_length is not None else self.max_len) - len(encoded_inputs["input_pids"])
              encoded_inputs["pmasks"] = [1] * len(encoded_inputs["input_pids"]) + [0] * difference
              encoded_inputs["input_pids"] = encoded_inputs["input_pids"] + [pad_token_aid] * difference

        if add_special_tokens:
            sequence = self.build_inputs_with_special_tokens(ids, pair_ids)
            token_type_ids = self.create_token_type_ids_from_sequences(ids, pair_ids)
            # added by Jong-Hoon Oh
            token_sequence = ["<CLS>"] + ftokens + ["<SEP>"]
            token_sequence_upper = ["<CLS>"] + ftokens_upper + ["<SEP>"]
        else:
            sequence = ids + pair_ids if pair else ids
            token_type_ids = [0] * len(ids) + ([1] * len(pair_ids) if pair else [])
            # added by Jong-Hoon Oh
            token_sequence = ftokens
            token_sequence_upper = ftokens_upper

        if return_special_tokens_mask:
            encoded_inputs["special_tokens_mask"] = self.get_special_tokens_mask(ids, pair_ids)

        encoded_inputs["input_ids"] = sequence
        encoded_inputs["tokens"] = token_sequence
        encoded_inputs["tokens_upper"] = token_sequence_upper
        if return_token_type_ids:
            encoded_inputs["token_type_ids"] = token_type_ids

        if max_length and len(encoded_inputs["input_ids"]) > max_length:
            encoded_inputs["input_ids"] = encoded_inputs["input_ids"][:max_length]
            encoded_inputs["tokens"] = encoded_inputs["tokens"][:max_length]
            encoded_inputs["tokens_upper"] = encoded_inputs["tokens_upper"][:max_length]
            # added by Jong-Hoon Oh
            if return_token_type_ids:
        ...
        if needs_to_be_padded:
            ...
            if self.padding_side == "right":
                ...
                # added by Jong-Hoon Oh
                encoded_inputs["tokens"] = encoded_inputs["tokens"] + ["<pad>"] * difference
                encoded_inputs["tokens_upper"] = encoded_inputs["tokens_upper"] + ["<pad>"] * difference
                ...
                # added by Jong-Hoon Oh
                encoded_inputs["tokens"] = ["<pad>"] * difference + encoded_inputs["tokens"]
                encoded_inputs["tokens_upper"] = ["<pad>"] * difference + encoded_inputs["tokens_upper"]
            else:
        ...
        if return_tensors == "tf" and is_tf_available():
            ...
            encoded_inputs["input_qids"] = torch.tensor([encoded_inputs["input_qids"]])
            encoded_inputs["input_pids"] = torch.tensor([encoded_inputs["input_pids"]])
            encoded_inputs["qmasks"] = torch.tensor([encoded_inputs["qmasks"]])
            encoded_inputs["pmasks"] = torch.tensor([encoded_inputs["pmasks"]])
    ...
    # added by Jong-Hoon Oh
    def truncate_sequences_for_cnn(
        self, ids, pair_ids=None, pair_tokens=None, pair_tokens_upper=None, num_tokens_to_remove=0, truncation_strategy="longest_first", stride=0
    ):
        """Truncates a sequence pair in place to the maximum length.
            truncation_strategy: string selected in the following options:
                - 'longest_first' (default) Iteratively reduce the inputs sequence until the input is under max_length
        ...
        overflowing_tokens_upper = []
        if truncation_strategy == "longest_first":
            # question or text_a was truncated here (longest_first by default)
            overflowing_tokens = []
            for _ in range(num_tokens_to_remove):
                if pair_ids is None or len(ids) > len(pair_ids):
                    overflowing_tokens = [ids[-1]] + overflowing_tokens
            ...
            # BERTAC does not use this option
            assert len(ids) > num_tokens_to_remove
            window_len = min(len(ids), stride + num_tokens_to_remove)
            overflowing_tokens = ids[-window_len:]
            ids = ids[:-num_tokens_to_remove]
        elif truncation_strategy == "only_second": 
            # BERT's option for truncated
            # not pair_ids but pair_tokens were used for the overflowing_tokens
            assert pair_ids is not None and len(pair_ids) > num_tokens_to_remove
            window_len = min(len(pair_ids), stride + num_tokens_to_remove)
            # not pair_ids but pair_tokens were used for the overflowing_tokens
            overflowing_tokens = pair_tokens[-window_len:] 
            overflowing_tokens_upper = pair_tokens_upper[-window_len:] 
            pair_ids = pair_ids[:-num_tokens_to_remove]
        elif truncation_strategy == "do_not_truncate":
            raise ValueError("Input sequence are too long for max_length. Please select a truncation strategy.")
        else:
        ...
        return (ids, pair_ids, overflowing_tokens, overflowing_tokens_upper)

...
""" Modified from examples/run_squad.py in the original Huggingface Transformers for training/testing an answer span selector
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        results = evaluate(args, cnn_model, model, tokenizer, prefix=str(global_step))
...
# Modified by Jong-Hoon Oh
# - cnn_model should be given as an argument
def evaluate(args, cnn_model, model, tokenizer, prefix="test", pred_dev=True):
    # modified by Jong-Hoon Oh
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            if len(output) >= 5:
                ...
                # modified by Jong-Hoon Oh
                #  - modified from SquadResult
                result = OpenQAResult(unique_id, start_logits, end_logits)
...
# - cnn_stoi: vocab.stoi for pretrained CNNs 
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    # modified by Jong-Hoon Oh
    if evaluate:
       logger.info("Specified cached file %s for evaluation", cached_dset_file)
    else:
       logger.info("Specified cached file %s for train", cached_dset_file)

    # Init features and dataset from cache if it exists
    if os.path.exists(cached_dset_file) and not args.overwrite_cache:
        if args.train_with_possible and not evaluate:
           logger.info("Loading features from cached file %s", cached_pdset_file)
           dset = torch.load(cached_pdset_file) # use only the postive examples for training 
           dataset = dset["possible_dataset"]
        else:
           logger.info("Loading features from cached file %s", cached_dset_file)
           dset = torch.load(cached_dset_file) # use both positive and negative examples (with negative_)
           dataset = dset["dataset"]
        del dset
        if evaluate:  # evaluation needs features and examples
           feat_set = torch.load(cached_feat_file) # selector does not use possible_dataset
           features = feat_set["features"]
           del feat_set 
           ex_set = torch.load(cached_exset_file) # selector does not use possible_dataset
           examples = ex_set["examples"]
           del ex_set 
    else:
        if args.local_rank == 0 and not evaluate:
            # Make sure only the first process in distributed training process the dataset, and the others will use the cache
            torch.distributed.barrier()
        logger.info("No cached file at %s", cached_dset_file) # input_dir="." by defaults
...
def main():
    ...
    parser.add_argument(
        "--pred_dev",
        action="store_true",
        help="If true, the predict file is assumed as that for development data.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--do_eval", action="store_true", help="Whether to run eval on the dev set or the test set.")
    parser.add_argument(
    ...
    # warmup_steps were replaced with warmup_ratio
    ...
    args.train_with_possible=True

    assert args.train_with_possible != args.version_2_with_negative
    ...
    # Here, loading the pretrained CNN
    ...
    # delete all_tokens and prep_tokens (never used later)
    del all_tokens
    del prep_tokens 
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if args.fp16:
                ...
                # amp.initialize should be called once
                model = amp.initialize(model, opt_level=args.fp16_opt_level)

            # Evaluate
            if global_step == "":
               prefix_str = "dev" if args.pred_dev else "test"
               result = evaluate(args, cnn_model, model, tokenizer, prefix=prefix_str)
            else:
               result = evaluate(args, cnn_model, model, tokenizer, prefix=global_step)

...
if($#ARGV != 3){
	print "perl run_mergemasked.pl set_stem[searchqa|quasart] script_dir tmp_dir num_parallel_processes\n";
	exit;
}
my $set = shift @ARGV;
my $pdir = shift @ARGV;
...
my $idir = "$tmp_dir/cand";
my $odir = "$tmp_dir/masked";
...
for(my $i = 0; $i <= $#dirs; $i++){
	if($dirs[$i] =~ /^($set.(?:train|dev|test))\.([0-9]+)/){
		...
		my $stem1 = $1;
		my $stem2 = $2;
		my $cmd  = "perl $pdir/merge_masked_data.pl $stem1.$stem2 $tmp_dir &";
		push @cmd_set, $cmd;
